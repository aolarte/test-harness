...
public class ZooKeeperClient {
    ...
    public static void main(String... args) throws IOException, InterruptedException, KeeperException {
        ...
        ZooKeeperClient clientTest=new ZooKeeperClient();
        clientTest.connect("127.0.0.1:9981");


        String path = "/node";
        byte[] data = "My Test Data".getBytes();

        if (clientTest.nodeExists(path)==null) {
            LOG.info("Create Node");
            clientTest.create(path, data);
        }

        LOG.info("Data: " +clientTest.getData(path));

        clientTest.updateData(path,"new Data".getBytes());

        clientTest.deleteNode(path);
