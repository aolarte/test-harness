...
-module(msg_server).         % Nothing new in this section except for the
...
-export([                      % Here we define our API functions as before
  ...
  get_count/0,
  register/2,
  send/2]).               % - returns the count state
...
handle_call(get_count, _From, #state{count=Count , subs=Subs}) ->
    {reply,
     Count,                    % here we synchronously respond with Count
     #state{count=Count+1, subs=Subs}     % and also update state
    }.
...
handle_cast({send, Name, Text}, State ) ->
  io:format("Send~n"),
  Map1 = State#state.subs,
  Destination = maps:get(Name,Map1),
  Destination ! {send,Text},
  {noreply,State};
handle_cast({register, Name, Destination}, State ) ->
  io:format("Register~n"),
  Map1 = State#state.subs,
  Map2 = Map1#{Name => Destination},
  io:format("Register ~w ~n",[maps:size(Map2)]),
  {noreply,
    #state{count=
    State#state.count+1, subs=Map2}
  };
handle_cast(say_hello, State) -> % ... becuase the second clause is here to
    ...
    #state{count=
      State#state.count+1, subs=State#state.subs}
